...
| 2 | Reconcile data and business context across heterogeneous sources | `whychain/ingest/warehouse.py`, `whychain/reconcile/`, `contracts/*.yml` (`calculation.canonical_sql`, `lineage`, `freshness_sla`, `reconciliation`) | Console section **Where the data came from** for grain and cadence: five sources, four cadences, per-source SLA verdict, each KPI read through its own contract. And a second system posting the same quantity: `finance_ledger` nets returns at posting date rather than sale date, so the two sit 2.1% apart on a median day, and the contract declares the tolerance. Select **North, 10–12 June 2026** to see the planted extract break: the two disagree by 39%, the verdict is `contradicted` rather than `unknown`, and no cause is proposed | Built |
| 3 | Identify and rank explanatory drivers using appropriate methods | `whychain/decompose/` (price/volume/mix identity), `whychain/rank/` (two tracks), `whychain/verify/` (event-time isolation, DiD, placebo, exposure consistency) | Console section **How we chose between explanations**. Track A is exact and its rows may be stated (precondition: SKU net units >= 0; net returns exceeding sales violate the identity and refuse); Track B is a standardised ridge and every row is labelled `CORRELATIONAL` and barred from becoming a cause | Built |
...
| 5 | Communicate uncertainty and abstain when evidence is insufficient **or contradictory** | `whychain/confidence/score.py` (banding, abstention), `whychain/confidence/calibrate.py` (isotonic, held-out), `whychain/reconcile/`, `whychain/narrate/nextcheck.py` | Select **South**, period 90 days: the engine reports UNKNOWN with what it ruled out and the next check, which the model writes from the shape of that particular failure, behind four deterministic gates that drop any sentence asserting a cause, naming something the run does not contain, or printing a figure absent from the facts. Measured: abstention precision **100%** (26 Sep, after B-070). Contradiction is a separate verdict with a separate next step: **North, 10–12 June 2026** returns `contradicted` and sends the reader to data engineering rather than to a commercial explanation | Built |
...
| 8 | Realistic security, cost, latency and scalability constraints | `whychain/telemetry/`, `AccessPolicy` in contracts, `whychain/personas/`, `bench/scale.py` | Console section **Run receipt** and the margin column: per-stage latency, model calls, tokens, rupee cost. p95 **0.18s** per diagnosis. Materialising lineage at ingest keeps 16x row scaling flat at 1.5x (down from 26.6x); under 16 concurrent readers throughput is flat at ~1.7 req/s because warehouse work is single-process | Built; scalability now measured rather than argued |
...
| 7 | One role-based security or entitlement scenario | Rail control **Entitlement**. Querying a region outside scope (e.g. South reader requesting West) returns HTTP 403 refusal before computation (?demo=blanked). For permitted queries, verified causes outside scope are withheld with a notice stating how many were withheld, what they were worth, and which role to escalate to. Row filter, column masks and a PII domain restriction are declared per contract | Met |
...
| | |
|---|---|
| **Top-1 among movements worth explaining** | **75.9%** (66 of 87) |
| Top-1 over the whole population | 45.8% |
| True cause verified at all | 47.9% |
| **False alarms on noise-only cases** | **0.0%** |
| Planted correlation traps rejected | 92.2% |
| **Cases needing an abstention that got one** | **100%** (17 of 17) |
| Abstentions that were right | 100% |
| Expected calibration error | 0.116 raw, **0.023 calibrated** on held out |
...
- **Materialised transforms at ingest resolved the row-scaling wall; concurrency limit remains.** `make scale` verified that pre-materialising `_prepared_pos_txn` drops the 16x read cost from 26.6x to 1.5x. Concurrency, measured again on 26 Sep: one process serves about 5 diagnoses a second however many readers arrive; several workers are now safe (the audit chain and feedback log hold across processes, B-077) and four serve 13.7 a second. The container runs `WHYCHAIN_WORKERS` of them; the desktop app runs one. Nothing has been run against a warehouse that does not fit on one machine.
