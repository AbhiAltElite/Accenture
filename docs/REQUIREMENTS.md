...
- **Materialised transforms at ingest resolved the row-scaling wall; concurrency limit remains.** `make scale` verified that pre-materialising `_prepared_pos_txn` drops the 16x read cost from 26.6x to 1.5x. The remaining measured limit is concurrency: the engine serves about 1.7 diagnoses per second however many readers arrive, because it is one process with one warehouse connection; multiple workers and an externalised cache are the route out, and are not built. Nothing has been run against a warehouse that does not fit on one machine.
